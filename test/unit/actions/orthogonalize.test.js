...
test.todo('actionOrthogonalize', async t => {
    var projection = {
        project: function (val) { return val; },
        invert: function (val) { return val; }
    };

    it('closed paths', function () {
        ...
        it('preserves the shape of skinny quads', function () {
            ...
            for (var i = 0; i < tests.length; i++) {
                ...
                var initialWidth = Rapid.sdk.geoSphericalDistance(graph.entity('a').loc, graph.entity('b').loc);
                graph = Rapid.actionOrthogonalize('-', projection)(graph);
                var finalWidth = Rapid.sdk.geoSphericalDistance(graph.entity('a').loc, graph.entity('b').loc);
