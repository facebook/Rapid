describe('osmRelation', function () {
    it('returns a relation', function () {
        expect(Rapid.osmRelation()).to.be.an.instanceOf(Rapid.osmRelation);
        expect(Rapid.osmRelation().type).to.equal('relation');
    });

    it('defaults members to an empty array', function () {
        expect(Rapid.osmRelation().members).to.eql([]);
    });

    it('sets members as specified', function () {
        expect(Rapid.osmRelation({members: ['n-1']}).members).to.eql(['n-1']);
    });

    it('defaults tags to an empty object', function () {
        expect(Rapid.osmRelation().tags).to.eql({});
    });

    it('sets tags as specified', function () {
        expect(Rapid.osmRelation({tags: {foo: 'bar'}}).tags).to.eql({foo: 'bar'});
    });

    describe('#copy', function () {
        it('returns a new Relation', function () {
            var r = Rapid.osmRelation({id: 'r'});
            var result = r.copy(null, {});

            expect(result).to.be.an.instanceof(Rapid.osmRelation);
        ...
        it('deep copies members', function () {
            ...
            var c = Rapid.osmNode({id: 'c'});
            var w = Rapid.osmWay({id: 'w', nodes: ['a','b','c','a']});
            var r = Rapid.osmRelation({id: 'r', members: [{id: 'w', role: 'outer'}]});
            var graph = new Rapid.Graph([a, b, c, w, r]);
            var copies = {};
            var result = r.copy(graph, copies);

            expect(Object.keys(copies)).to.have.length(5);
            expect(copies.w).to.be.an.instanceof(Rapid.osmWay);
            expect(copies.a).to.be.an.instanceof(Rapid.osmNode);
            expect(copies.b).to.be.an.instanceof(Rapid.osmNode);
            expect(copies.c).to.be.an.instanceof(Rapid.osmNode);
        ...
        it('deep copies non-tree relation graphs without duplicating children', function () {
            ...
            var r1 = Rapid.osmRelation({id: 'r1', members: [{id: 'r2'}, {id: 'w'}]});
            var r2 = Rapid.osmRelation({id: 'r2', members: [{id: 'w'}]});
            var graph = new Rapid.Graph([w, r1, r2]);
            var copies = {};
            r1.copy(graph, copies);

            expect(Object.keys(copies)).to.have.length(3);
            expect(copies.r1).to.be.an.instanceof(Rapid.osmRelation);
            expect(copies.r2).to.be.an.instanceof(Rapid.osmRelation);
            expect(copies.w).to.be.an.instanceof(Rapid.osmWay);
        ...
        it('deep copies cyclical relation graphs without issue', function () {
            var r1 = Rapid.osmRelation({id: 'r1', members: [{id: 'r2'}]});
            var r2 = Rapid.osmRelation({id: 'r2', members: [{id: 'r1'}]});
            var graph = new Rapid.Graph([r1, r2]);
            var copies = {};
            r1.copy(graph, copies);
        ...
        it('deep copies self-referencing relations without issue', function () {
            var r = Rapid.osmRelation({id: 'r', members: [{id: 'r'}]});
            var graph = new Rapid.Graph([r]);
            var copies = {};
            r.copy(graph, copies);
    ...
    describe('#extent', function () {
        it('returns the minimal extent containing the extents of all members', function () {
            var a = Rapid.osmNode({loc: [0, 0]});
            var b = Rapid.osmNode({loc: [5, 10]});
            var r = Rapid.osmRelation({members: [{id: a.id}, {id: b.id}]});
            var graph = new Rapid.Graph([a, b, r]);
            expect(r.extent(graph)).to.eql(new sdk.Extent([0, 0], [5, 10]));
        });
        ...
        it('returns the known extent of incomplete relations', function () {
            ...
            var b = Rapid.osmNode({loc: [5, 10]});
            var r = Rapid.osmRelation({members: [{id: a.id}, {id: b.id}]});
            var graph = new Rapid.Graph([a, r]);
            expect(r.extent(graph)).to.eql(new sdk.Extent([0, 0], [0, 0]));
        });
        ...
        it('does not error on self-referencing relations', function () {
            ...
            var graph = new Rapid.Graph([r]);
            expect(r.extent(graph)).to.eql(new sdk.Extent());
        });
    });

    describe('#geometry', function () {
        it('returns \'area\' for multipolygons', function () {
            expect(Rapid.osmRelation({tags: {type: 'multipolygon'}}).geometry(new Rapid.Graph())).to.equal('area');
        });

        it('returns \'relation\' for other relations', function () {
            expect(Rapid.osmRelation().geometry(new Rapid.Graph())).to.equal('relation');
        });
    });

    describe('#isDegenerate', function () {
        it('returns true for a relation without members', function () {
            expect(Rapid.osmRelation().isDegenerate()).to.equal(true);
        });

        it('returns false for a relation with members', function () {
            expect(Rapid.osmRelation({members: [{id: 'a', role: 'inner'}]}).isDegenerate()).to.equal(false);
        });
    });

    describe('#memberByRole', function () {
        it('returns the first member with the given role', function () {
            var r = Rapid.osmRelation({members: [
                {id: 'a', role: 'inner'},
                {id: 'b', role: 'outer'},
                {id: 'c', role: 'outer'}]});
        ...
        it('returns undefined if no members have the given role', function () {
            expect(Rapid.osmRelation().memberByRole('outer')).to.be.undefined;
        });
    });
    ...
    describe('#memberById', function () {
        ...
        it('returns undefined if no members have the given role', function () {
            expect(Rapid.osmRelation().memberById('b')).to.be.undefined;
        });
    });
    ...
    describe('#isRestriction', function () {
        it('returns true for \'restriction\' type', function () {
            expect(Rapid.osmRelation({tags: {type: 'restriction'}}).isRestriction()).to.be.true;
        });

        it('returns true for \'restriction:type\' types', function () {
            expect(Rapid.osmRelation({tags: {type: 'restriction:bus'}}).isRestriction()).to.be.true;
        });

        it('returns false otherwise', function () {
            expect(Rapid.osmRelation().isRestriction()).to.be.false;
            expect(Rapid.osmRelation({tags: {type: 'multipolygon'}}).isRestriction()).to.be.false;
        });
    });

    describe('#isValidRestriction', function () {
        it('not a restriction', function () {
            var r = Rapid.osmRelation({ id: 'r', tags: { type: 'multipolygon' }});
            var graph = new Rapid.Graph([r]);
            expect(r.isValidRestriction(graph)).to.be.false;
        });

        it('typical restriction (from way, via node, to way) is valid', function () {
            var f = Rapid.osmWay({id: 'f'});
            var v = Rapid.osmNode({id: 'v'});
            var t = Rapid.osmWay({id: 't'});
            var r = Rapid.osmRelation({
                id: 'r',
        ...
        it('multiple froms, normal restriction is invalid', function () {
            ...
            var graph = new Rapid.Graph([f1, f2, v, t, r]);
        ...
        it('multiple tos, normal restriction is invalid', function () {
            var f = Rapid.osmWay({id: 'f'});
            var v = Rapid.osmNode({id: 'v'});
            var t1 = Rapid.osmWay({id: 't1'});
            var t2 = Rapid.osmWay({id: 't2'});
            var r = Rapid.osmRelation({
                id: 'r',
                tags: { type: 'restriction', restriction: 'no_left_turn' },
                members: [
            ...
            var graph = new Rapid.Graph([f, v, t1, t2, r]);
        ...
        it('multiple vias, with some as node is invalid', function () {
            var f = Rapid.osmWay({id: 'f'});
            var v1 = Rapid.osmNode({id: 'v1'});
            var v2 = Rapid.osmWay({id: 'v2'});
            var t = Rapid.osmWay({id: 't'});
            var r = Rapid.osmRelation({
                id: 'r',
                tags: { type: 'restriction', restriction: 'no_left_turn' },
            ...
            var graph = new Rapid.Graph([f, v1, v2, t, r]);
    ...
    describe('#addMember', function () {
        ...
        it('adds a member at index 0', function () {
            var r = Rapid.osmRelation({members: [{id: '1'}]});
    ...
    describe('#updateMember', function () {
        it('updates the properties of the relation member at the specified index', function () {
            var r = Rapid.osmRelation({members: [{role: 'forward'}]});
    ...
    describe('#removeMember', function () {
        it('removes the member at the specified index', function () {
            var r = Rapid.osmRelation({members: [{id: 'a'}, {id: 'b'}, {id: 'c'}]});
    ...
    describe('#removeMembersWithID', function () {
        it('removes members with the given ID', function () {
            var r = Rapid.osmRelation({members: [{id: 'a'}, {id: 'b'}, {id: 'a'}]});
    ...
    describe('#asJXON', function () {
        it('converts a relation to jxon', function() {
            var relation = Rapid.osmRelation({id: 'r-1', members: [{id: 'w1', role: 'forward', type: 'way'}], tags: {type: 'route'}});
        ...
        it('includes changeset if provided', function() {
            expect(Rapid.osmRelation().asJXON('1234').relation['@changeset']).to.equal('1234');
        });
    });

    describe('#asGeoJSON', function () {
        it('converts a multipolygon to a GeoJSON MultiPolygon geometry', function() {
            var a = Rapid.osmNode({loc: [1, 1]});
            var b = Rapid.osmNode({loc: [3, 3]});
            var c = Rapid.osmNode({loc: [2, 2]});
            var w = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var r = Rapid.osmRelation({tags: {type: 'multipolygon'}, members: [{id: w.id, type: 'way'}]});
            var g = new Rapid.Graph([a, b, c, w, r]);
        ...
        it('forces clockwise winding order for outer multipolygon ways', function() {
            var a = Rapid.osmNode({loc: [0, 0]});
            var b = Rapid.osmNode({loc: [0, 1]});
            var c = Rapid.osmNode({loc: [1, 0]});
            var w = Rapid.osmWay({nodes: [a.id, c.id, b.id, a.id]});
            var r = Rapid.osmRelation({tags: {type: 'multipolygon'}, members: [{id: w.id, type: 'way'}]});
            var g = new Rapid.Graph([a, b, c, w, r]);
            var json = r.asGeoJSON(g);
        ...
        it('forces counterclockwise winding order for inner multipolygon ways', function() {
            ...
            var b = Rapid.osmNode({loc: [0, 1]});
            var c = Rapid.osmNode({loc: [1, 0]});
            var d = Rapid.osmNode({loc: [0.1, 0.1]});
            var e = Rapid.osmNode({loc: [0.1, 0.2]});
            var f = Rapid.osmNode({loc: [0.2, 0.1]});
            var outer = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var inner = Rapid.osmWay({nodes: [d.id, e.id, f.id, d.id]});
            var r = Rapid.osmRelation({members: [{id: outer.id, type: 'way'}, {id: inner.id, role: 'inner', type: 'way'}]});
            var g = new Rapid.Graph([a, b, c, d, e, f, outer, inner, r]);
        ...
        it('converts a relation to a GeoJSON FeatureCollection', function() {
            var a = Rapid.osmNode({loc: [1, 1]});
            var r = Rapid.osmRelation({tags: {type: 'type'}, members: [{id: a.id, role: 'role'}]});
            var g = new Rapid.Graph([a, r]);
            var json = r.asGeoJSON(g);
    ...
    describe('#multipolygon', function () {
        specify('single polygon consisting of a single way', function () {
            ...
            var c = Rapid.osmNode({loc: [2, 2]});
            var w = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var r = Rapid.osmRelation({members: [{id: w.id, type: 'way'}]});
            var g = new Rapid.Graph([a, b, c, w, r]);
        ...
        specify('single polygon consisting of multiple ways', function () {
            ...
            var b = Rapid.osmNode({loc: [3, 3]});
            var c = Rapid.osmNode({loc: [2, 2]});
            var w1 = Rapid.osmWay({nodes: [a.id, b.id]});
            var w2 = Rapid.osmWay({nodes: [b.id, c.id, a.id]});
            var r  = Rapid.osmRelation({members: [{id: w1.id, type: 'way'}, {id: w2.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, w1, w2, r]);
        ...
        specify('single polygon consisting of multiple ways, one needing reversal', function () {
            ...
            var c  = Rapid.osmNode({loc: [2, 2]});
            var w1 = Rapid.osmWay({nodes: [a.id, b.id]});
            var w2 = Rapid.osmWay({nodes: [a.id, c.id, b.id]});
            var r  = Rapid.osmRelation({members: [{id: w1.id, type: 'way'}, {id: w2.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, w1, w2, r]);
        ...
        specify('multiple polygons consisting of single ways', function () {
            ...
            var b  = Rapid.osmNode({loc: [3, 3]});
            var c  = Rapid.osmNode({loc: [2, 2]});
            var d  = Rapid.osmNode({loc: [4, 4]});
            var e  = Rapid.osmNode({loc: [6, 6]});
            var f  = Rapid.osmNode({loc: [5, 5]});
            var w1 = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var w2 = Rapid.osmWay({nodes: [d.id, e.id, f.id, d.id]});
            var r  = Rapid.osmRelation({members: [{id: w1.id, type: 'way'}, {id: w2.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, d, e, f, w1, w2, r]);
        ...
        specify('invalid geometry: unclosed ring consisting of a single way', function () {
            ...
            var b = Rapid.osmNode({loc: [3, 3]});
            var c = Rapid.osmNode({loc: [2, 2]});
            var w = Rapid.osmWay({nodes: [a.id, b.id, c.id]});
            var r = Rapid.osmRelation({members: [{id: w.id, type: 'way'}]});
            var g = new Rapid.Graph([a, b, c, w, r]);
        ...
        specify('invalid geometry: unclosed ring consisting of multiple ways', function () {
            ...
            var c  = Rapid.osmNode({loc: [2, 2]});
            var w1 = Rapid.osmWay({nodes: [a.id, b.id]});
            var w2 = Rapid.osmWay({nodes: [b.id, c.id]});
            var r  = Rapid.osmRelation({members: [{id: w1.id, type: 'way'}, {id: w2.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, w1, w2, r]);
        ...
        specify('invalid geometry: unclosed ring consisting of multiple ways, alternate order', function () {
            var a  = Rapid.osmNode({loc: [1, 1]});
            var b  = Rapid.osmNode({loc: [2, 2]});
            var c  = Rapid.osmNode({loc: [3, 3]});
            var d  = Rapid.osmNode({loc: [4, 4]});
            var w1 = Rapid.osmWay({nodes: [c.id, d.id]});
            var w2 = Rapid.osmWay({nodes: [a.id, b.id, c.id]});
            var r  = Rapid.osmRelation({members: [{id: w1.id, type: 'way'}, {id: w2.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, d, w1, w2, r]);
        ...
        specify('invalid geometry: unclosed ring consisting of multiple ways, one needing reversal', function () {
            ...
            var c  = Rapid.osmNode({loc: [3, 3]});
            var d  = Rapid.osmNode({loc: [4, 4]});
            var w1 = Rapid.osmWay({nodes: [a.id, b.id, c.id]});
            var w2 = Rapid.osmWay({nodes: [d.id, c.id]});
            var r  = Rapid.osmRelation({members: [{id: w1.id, type: 'way'}, {id: w2.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, d, w1, w2, r]);
        ...
        specify('invalid geometry: unclosed ring consisting of multiple ways, one needing reversal, alternate order', function () {
            ...
            var d  = Rapid.osmNode({loc: [4, 4]});
            var w1 = Rapid.osmWay({nodes: [c.id, d.id]});
            var w2 = Rapid.osmWay({nodes: [c.id, b.id, a.id]});
            var r  = Rapid.osmRelation({members: [{id: w1.id, type: 'way'}, {id: w2.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, d, w1, w2, r]);
        ...
        specify('single polygon with single single-way inner', function () {
            ...
            var c = Rapid.osmNode({loc: [1, 0]});
            var d = Rapid.osmNode({loc: [0.1, 0.1]});
            var e = Rapid.osmNode({loc: [0.2, 0.1]});
            var f = Rapid.osmNode({loc: [0.1, 0.2]});
            var outer = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var inner = Rapid.osmWay({nodes: [d.id, e.id, f.id, d.id]});
            var r = Rapid.osmRelation({members: [
        ...
        specify('single polygon with single multi-way inner', function () {
            ...
            var f = Rapid.osmNode({loc: [0.2, 0.1]});
            var outer = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var inner1 = Rapid.osmWay({nodes: [d.id, e.id]});
            var inner2 = Rapid.osmWay({nodes: [e.id, f.id, d.id]});
            var r = Rapid.osmRelation({members: [
                {id: outer.id, type: 'way'},
                {id: inner1.id, role: 'inner', type: 'way'},
                {id: inner2.id, role: 'inner', type: 'way'}
            ]});
            var graph = new Rapid.Graph([a, b, c, d, e, f, outer, inner1, inner2, r]);
        ...
        specify('single polygon with multiple single-way inners', function () {
            ...
            var e = Rapid.osmNode({loc: [0.2, 0.1]});
            var f = Rapid.osmNode({loc: [0.1, 0.2]});
            var g = Rapid.osmNode({loc: [0.2, 0.2]});
            var h = Rapid.osmNode({loc: [0.3, 0.2]});
            var i = Rapid.osmNode({loc: [0.2, 0.3]});
            var outer = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var inner1 = Rapid.osmWay({nodes: [d.id, e.id, f.id, d.id]});
            var inner2 = Rapid.osmWay({nodes: [g.id, h.id, i.id, g.id]});
            var r = Rapid.osmRelation({members: [
                {id: outer.id, type: 'way'},
                {id: inner1.id, role: 'inner', type: 'way'},
                {id: inner2.id, role: 'inner', type: 'way'}
            ]});
            var graph = new Rapid.Graph([a, b, c, d, e, f, g, h, i, outer, inner1, inner2, r]);
        ...
        specify('multiple polygons with single single-way inner', function () {
            ...
            var e = Rapid.osmNode({loc: [0.2, 0.1]});
            var f = Rapid.osmNode({loc: [0.1, 0.2]});
            var g = Rapid.osmNode({loc: [0, 0]});
            var h = Rapid.osmNode({loc: [0, -1]});
            var i = Rapid.osmNode({loc: [-1, 0]});
            var outer1 = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var outer2 = Rapid.osmWay({nodes: [g.id, h.id, i.id, g.id]});
            var inner = Rapid.osmWay({nodes: [d.id, e.id, f.id, d.id]});
            var r = Rapid.osmRelation({members: [
            ...
            var graph = new Rapid.Graph([a, b, c, d, e, f, g, h, i, outer1, outer2, inner, r]);
        ...
        specify('invalid geometry: unmatched inner', function () {
            ...
            var c = Rapid.osmNode({loc: [3, 3]});
            var w = Rapid.osmWay({nodes: [a.id, b.id, c.id, a.id]});
            var r = Rapid.osmRelation({members: [{id: w.id, role: 'inner', type: 'way'}]});
            var g = new Rapid.Graph([a, b, c, w, r]);
        ...
        specify('incomplete relation', function () {
            ...
            var w1 = Rapid.osmWay({nodes: [a.id, b.id, c.id]});
            var w2 = Rapid.osmWay();
            var r  = Rapid.osmRelation({members: [{id: w2.id, type: 'way'}, {id: w1.id, type: 'way'}]});
            var g  = new Rapid.Graph([a, b, c, w1, r]);
    ...
    describe('.creationOrder comparator', function () {
        specify('orders existing relations newest-first', function () {
            var a = Rapid.osmRelation({ id: 'r1' });
            var b = Rapid.osmRelation({ id: 'r2' });
            expect(Rapid.osmRelation.creationOrder(a, b)).to.be.above(0);
            expect(Rapid.osmRelation.creationOrder(b, a)).to.be.below(0);
        });

        specify('orders new relations newest-first', function () {
            var a = Rapid.osmRelation({ id: 'r-1' });
            var b = Rapid.osmRelation({ id: 'r-2' });
            expect(Rapid.osmRelation.creationOrder(a, b)).to.be.above(0);
            expect(Rapid.osmRelation.creationOrder(b, a)).to.be.below(0);
        });
    });
