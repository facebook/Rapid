...
test.todo('actionCircularize', async t => {
    var projection = new Rapid.sdk.Projection().scale(150);
    ...
    function isCircular(id, graph) {
        var points = graph.childNodes(graph.entity(id))
            ...
            centroid = Rapid.d3.polygonCentroid(points),
            radius = Rapid.sdk.vecLength(centroid, points[0]),
    ...
    function area(id, graph) {
        ...
        return Rapid.d3.polygonArea(points);
    }
    ...
    it('limits movement of nodes that are members of other ways', function () {
        ...
        expect(isCircular('-', graph)).to.be.ok;
        expect(Rapid.sdk.vecLength(graph.entity('d').loc, [2, -2])).to.be.lt(0.5);
    });
