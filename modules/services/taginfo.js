...
const APIROOT = 'https://taginfo.openstreetmap.org/api/4/';
...
export default {
  ...
  keys: function(params, callback) {
    ...
    const url = APIROOT + 'keys/all?' + utilQsString(params);
    doRequest(url, params, false, callback, (err, d) => {
      if (err) {
        callback(err);
      } else {
  ...
  multikeys: function(params, callback) {
    ...
    const url = APIROOT + 'keys/all?' + utilQsString(params);
    doRequest(url, params, true, callback, (err, d) => {
      if (err) {
        callback(err);
      } else {
  ...
  values: function(params, callback) {
    ...
    const url = APIROOT + 'key/values?' + utilQsString(params);
    doRequest(url, params, false, callback, (err, d) => {
      if (err) {
        callback(err);
      } else {
        ...
        const re = /network|taxon|genus|species|brand|grape_variety|royal_cypher|listed_status|booth|rating|stars|:output|_hours|_times|_ref|manufacturer|country|target|brewery/;
  ...
  roles: function(params, callback) {
    ...
    const url = APIROOT + 'relation/roles?' + utilQsString(params);
    doRequest(url, params, true, callback, (err, d) => {
      if (err) {
        callback(err);
      } else {
  ...
  docs: function(params, callback) {
    ...
    const url = APIROOT + path + utilQsString(params);
    doRequest(url, params, true, callback, (err, d) => {
      if (err) {
        callback(err);
      } else {
