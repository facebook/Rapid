...
export function validationMissingRole() {
  ...
  let validation = function checkMissingRole(entity, graph) {
    let issues = [];
    if (entity.type === 'way') {
      graph.parentRelations(entity).forEach(relation => {
      ...
      entity.indexedMembers().forEach(member => {
    ...
    return new validationIssue({
      ...
      message: (context) => {
        ...
        return (member && relation) ? t.html('issues.missing_role.message', {
          member: utilDisplayLabel(context, member, context.graph()),
          relation: utilDisplayLabel(context, relation, context.graph())
        }) : '';
      },
