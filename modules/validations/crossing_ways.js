...
export function validationCrossingWays(context) {
  ...
  function hasTag(tags, k) {
    return tags[k] !== undefined && tags[k] !== 'no';
  }
  ...
  function allowsBridge(type) {
    return type === 'highway' || type === 'railway' || type === 'waterway' || type === 'aeroway';
  }
  function allowsTunnel(type) {
    return type === 'highway' || type === 'railway' || type === 'waterway';
  }
  ...
  function isLegitCrossing(tags1, type1, tags2, type2) {
    ...
    if ((type1 === 'highway' && type2 === 'highway') && layer1 !== layer2) {
        // assume highways don't interact if they're on different layers
        return true;
    }

    if (allowsBridge(type1) && allowsBridge(type2)) {
    ...
    } else if (allowsBridge(type1) && hasTag(tags1, 'bridge')) return true;
    else if (allowsBridge(type2) && hasTag(tags2, 'bridge')) return true;

    if (allowsTunnel(type1) && allowsTunnel(type2)) {
    ...
    } else if (allowsTunnel(type1) && hasTag(tags1, 'tunnel')) return true;
    else if (allowsTunnel(type2) && hasTag(tags2, 'tunnel')) return true;
    ...
    if (type1 === 'building' || type2 === 'building') {
   ...
   * This determines whether a crossing between the given entities is allowed,
   * and if so, what tags should be suggested on the crossing node.
   * @param  {Entity} entity1
   * @param  {Entity} entity2
   * @param  {Graph}  graph
   * @return {Object|null} the tags for the connecting node, or null if the entities should not be joined
   */
  function tagsForConnectionNodeIfAllowed(entity1, entity2, graph) {
    ...
    const crossing = [type1, type2].sort().join('-');  // a string like 'highway-highway'
    ...
    if (crossing === 'aeroway-aeroway') {
      return {};  // allowed, no tag suggestion

    } else if (crossing === 'aeroway-highway') {
    ...
    } else if (crossing === 'aeroway-railway') {
      return { aeroway: 'aircraft_crossing', railway: 'level_crossing' };

    } else if (crossing === 'aeroway-waterway') {
      return null;  // not allowed

    } else if (crossing === 'highway-highway') {
      ...
      if ((entity1IsPath || entity2IsPath) && entity1IsPath !== entity2IsPath) {
        ...
        if (!bothLines || roadFeature.tags.highway === 'track') {
          return {};  // allowed, no tag suggestion
        }
    ...
    } else if (crossing === 'highway-railway') {
      if (!bothLines) {
        return {};  // allowed, no tag suggestion
      }
    ...
    } else if (crossing === 'highway-waterway') {
      ...
      if (disallowFord[entity1.tags.highway] || disallowFord[entity2.tags.highway]) {
        return null;
      }
      return bothLines ? { ford: 'yes' } : {};

    } else if (crossing === 'railway-railway') {
      return {};  // allowed, no tag suggestion

    } else if (crossing === 'railway-waterway') {
      return null;  // not allowed

    } else if (crossing === 'waterway-waterway') {
      return {};  // allowed, no tag suggestion
    }
