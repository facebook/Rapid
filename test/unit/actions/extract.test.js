...
describe('actionExtract', () => {
  ...
  it('extracts a node from the graph', () => {
    ...
    const action = Rapid.actionExtract('n1', projection);
    graph = action(graph);
  ...
  it('extracts a way from the graph', () => {
    ...
    const action = Rapid.actionExtract('w1', projection);
    graph = action(graph);
    const extractedNodeID = action.getExtractedNodeID();
    const extractedNode = graph.entity(extractedNodeID);
  ...
  it('handles a way with non-finite centroid', () => {
    ...
    // Mock projection.project to return non-finite values
    const originalProject = projection.project;
    projection.project = () => [Infinity, Infinity];

    const action = Rapid.actionExtract('w1', projection);
    graph = action(graph);

    // Restore the original projection.project function
    projection.project = originalProject;
