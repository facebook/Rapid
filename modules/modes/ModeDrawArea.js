...
export class ModeDrawArea extends AbstractMode {
  ...
  exit() {
    ...
    if (length < 4) {
      if (DEBUG) {
        console.log('ModeDrawArea: draw way invalid, rolling back');  // eslint-disable-line no-console
      }
  ...
  _move(eventData) {
    ...
    // Snap to a way
      ...
      const choice = geoChooseEdge(graph.childNodes(entity), coord, projection, this.drawNode.id);
...
if (choice && choice.distance < SNAP_DIST) {
  ...
  _clickWay(loc, edge) {
    ...
    if (this.drawWay) {
      ...
      if (DEBUG) {
        console.log(`ModeDrawArea: _clickWay, extending area to edge ${edge}`);  // eslint-disable-line no-console
      }
    ...
    // Start a new area at `loc` on target edge...
      ...
      if (DEBUG) {
        console.log(`ModeDrawArea: _clickWay, starting area at edge ${edge}`);  // eslint-disable-line no-console
      }
      this.firstNode = osmNode({ loc: loc });
      this.lastNode = this.firstNode;
      this.drawNode = osmNode({ loc: loc });
  ...
  _clickNode(loc, targetNode) {
    ...
    if (this.drawWay) {
      ...
      if (DEBUG) {
        console.log(`ModeDrawArea: _clickNode, extending area to ${targetNode.id}`);  // eslint-disable-line no-console
      }
  ...
  _finish() {
    if (this.drawWay) {
      if (DEBUG) {
        console.log(`ModeDrawArea: _finish, drawWay.id = ${this.drawWay.id}`);  // eslint-disable-line no-console
      }
      const context = this.context;
  ...
  _cancel() {
    if (DEBUG) {
      console.log(`ModeDrawArea: _cancel`); // eslint-disable-line no-console
    }
    this.drawWay = null;
    this.drawNode = null;
    this.firstNode = null;
